...
weight = {'o': 1.4554088613628926, '[C@]': 1.5435154133217646, '(': 1.1336555900413336, '2': 1.2271525729258035,
          '[NH+]': 1.4465062838107283, 'I': 1.6430956542751018, '[n-]': 1.6867424058204643, 'F': 1.3881706845994657,
          '[P@@]': 1.7699859535284932, '[O-]': 1.4710446425117816, 'O': 1.2114536486650351, '/': 1.46070186168524,
          '[S@@]': 1.635572566873708, 's': 1.4396772582455102, 'C': 1.0345098327192341, 'S': 1.4077282065547054,
          '[P@]': 1.7776429338887791, '[S-]': 1.6717204720988241, '=': 1.2422088634832562, '-': 1.442980341166416,
          '[S@]': 1.6344459064391046, '[nH+]': 1.5624020603137296, '4': 1.4558197854433321, '[o+]': 1.7963122129799949,
          '1': 1.1714583077479712, '[n+]': 1.6506253800862147, '[C@@H]': 1.3663339467456452, '7': 1.808367246924444,
          'n': 1.2919103820255748, '[N-]': 1.6289874483327829, '\\': 1.5535400325637365, 'Br': 1.5105718048132772,
          '[NH-]': 1.798927528777345, '6': 1.7293491866176853, '[C@@]': 1.5475291210954323, 'c': 1.0,
          'N': 1.2509856983582104, 'Cl': 1.4331804709686848, '[N+]': 1.5167331656034744, '5': 1.594380066179499,
          '[nH]': 1.4864410689971213, '3': 1.3302593095495208, ')': 1.1336555900413336, '#': 1.4985027709893173,
          '[NH3+]': 1.545441097578336, 'P': 1.7416132505327901, '[NH2+]': 1.4913053765510265,
          '[C@H]': 1.368650785106763}
...
class VAETrainer(TrainerBase):
    def __init__(self, args, vocab, vae, enc_optimizer, dec_optimizer, scheduler, train_bucket_iter, test_bucket_iter,
        ...
        self.pred_loss_function = ops.PropLoss()
    ...
    def run_epoch(self, kl_weight=1, epsilon_std=1.0):
        cnt = total_xent_loss = total_kl_loss = total_loss = total_prop_loss = 0
        self.vae.train()
        for batch in self.train_bucket_iter:
            ...
            x = batch.smile[0]
            lens = batch.smile[1]
            prop_target = batch.property.cuda().detach().float()
            # for target, remove the first <sos> token
            x_target = x[:, 1:].cuda().detach().long()

            self.enc_optimizer.zero_grad()
            self.dec_optimizer.zero_grad()
            if self.pred_optimizer is not None: self.pred_optimizer.zero_grad()

            x_hat, z_mean, z_log_var, z, prop_pred = self.vae(x, epsilon_std=epsilon_std)
            ...
            if self.pred_optimizer is not None:
                prop_loss = self.pred_loss_function.forward(prop_pred, prop_target)
                loss += (prop_loss * self.prop_weight)
            ...
            if self.pred_optimizer is not None:
                ...
                total_prop_loss += prop_loss.cpu().detach().numpy()

            total_xent_loss += xent_loss.cpu().detach().numpy()
            total_kl_loss += kl_loss.cpu().detach().numpy()
            cnt += x.size(0)

        return total_loss / cnt, total_xent_loss / cnt, total_kl_loss / cnt, total_prop_loss / cnt
    ...
    def run_epoch_train(self):
        ...
        for batch in self.test_bucket_iter:
            ...
            x_hat, z_mean, z_log_var, _, prop_pred = self.vae(x, epsilon_std=1.0)
    ...
    def validate(self, epsilon_std=1.0):
        ...
        with open(os.path.join(self.log_path, 'miss_pred.txt'), 'w') as f:
        ...
        for _ in range(40):
            ...
            for smi in smiles_pred:
                if smi is None:
                    continue
                elif len(smi) > 1 and Chem.MolFromSmiles(smi) is not None:
                    cnt_valid += 1
    ...
    def test(self, is_test_iter=True, epsilon_std=1.0):
        self.vae.eval()
        X = []
        z = None
        pred_data = None
        pred_target = None
        if is_test_iter:
            bucket_iter = self.test_bucket_iter
            print('Load validation dataset')
        else:
            bucket_iter = self.train_bucket_iter
            print('Load training dataset')
        with torch.no_grad():
            for batch in bucket_iter:
                x = batch.smile[0]
                x_target = x[:, 1:]
                # prop_target = batch.property.float()
                prop_target = batch.property

                x_hat, z_mean, z_log_var, _, prop_pred = self.vae(x, epsilon_std=1.0)
                # z = self.vae.encoder_sample(x, epsilon_std=epsilon_std)[-1]
                # prop_pred = self.vae.predictor(z)
                for i in range(x_target.shape[0]):
                    try:
                        smi = [self.vocab.itos[p] for p in x_target[i]]
                        smi = ''.join(smi).split()[0]
                        X.append(smi)
                    except:
                        X.append(None)
                pred_target = prop_target.data if pred_target is None else torch.cat((pred_target, prop_target.data))
                if self.pred_optimizer is not None:
                    pred_data = prop_pred.data if pred_data is None else torch.cat((pred_data, prop_pred.data))
                z = z_mean.data if z is None else torch.cat((z, z_mean.data))
        return pred_data, pred_target, z, X

    def train(self):
        results_fmt = ("{} :: {} :: loss {:.3f} xcent {:.3f} kl {:.3f} prop {:.6f}" + " ").format
        for self.epoch in range(self.num_epochs):
            epsilon_std = 1.0
            loss, xcent_loss, kl_loss, prop_loss = self.run_epoch(epsilon_std=epsilon_std)

            # if self.epoch % 2000 == 0 and self.epoch >= 2000:
            ...
            if self.pred_optimizer is not None: self.tensorboard.add_scalar('train/prop_loss', prop_loss, self.epoch)
            ...
            self.logger.info(results_fmt(time.strftime("%H:%M:%S"), self.epoch, loss, xcent_loss, kl_loss, prop_loss))
    ...
    def save(self, is_best, name):
        ...
        if self.pred_optimizer is not None:
            state.update({'pred_optimizer': self.pred_optimizer.state_dict()})
    ...
    def load_raw(self, path):
        ...
        # if self.pred_optimizer is not None:
        #     self.pred_optimizer.load_state_dict(state['pred_optimizer'])
