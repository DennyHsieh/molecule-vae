...
class Vae(nn.Module):
    def __init__(self, vocab, vocab_size, embedding_size, dropout, padding_idx,
            sos_idx, unk_idx, max_len, n_layers, hidden_size, prop_size, pred_depth=3,
            bidirectional=False, latent_size=128, partialsmiles=False, run_predictor=False):
        ...
        self.predictor = Predictor(latent_size=latent_size, output_size=prop_size, pred_depth=pred_depth)
        ...
        self.predictor_params = self.predictor.parameters()
    ...
    def forward(self, input_sequence, epsilon_std=1.0):
        ...
        # Predictor
        # if self.run_predictor: prop_val = self.predictor(z)
        if self.run_predictor: prop_val = self.predictor(mean)
        else: prop_val = None

        # decoder output projection on vacab
        outputs = self.decoder(input_embedding, z)
        return outputs, mean, logv, z, prop_val
...
class Predictor(nn.Module):
    def __init__(self, latent_size=56, output_size=1, nonlinearity=nn.ReLU, dropout=0.15, pred_depth=3):
        super(Predictor, self).__init__()
        self.dropout = dropout
        self.pred_depth = pred_depth

        # TODO: regularization(weight decay)
        self.hidden = nn.ModuleList()
        for depth in range(self.pred_depth):
            self.hidden.append(nn.Linear(int(latent_size * 0.5 ** depth), int(latent_size * 0.5 ** (depth + 1))))
            if nonlinearity is not None:
                self.hidden.append(nonlinearity())
            self.hidden.append(nn.Dropout(self.dropout))
            self.hidden.append(nn.BatchNorm1d(int(latent_size * 0.5 ** (depth + 1))))
        self.fc_out = nn.Linear(int(latent_size * 0.5 ** self.pred_depth), output_size)

    def forward(self, mu):
        for layer in self.hidden:
            mu = layer(mu)
        return self.fc_out(mu)
